...
The `home-routes` handler in the `todo-reframe.routes.home` namespace
...
The `home-page` function will in turn call the `todo-reframe.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/todo_reframe/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `todo-reframe.handler` namespace:
...
Request middleware functions are located under the `todo-reframe.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `todo-reframe.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `todo-reframe.core` in some other namespace.
